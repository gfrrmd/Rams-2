...
async def start_client_for_user(user_id, api_id, api_hash, string_session):
    ...
    async def auto_dl_handler(event):
        ...
        async with lock:
            try:
                status_msg = await client.send_message("me", "⏱️ Auto DL terdeteksi... 0.00%")
                media_bytes = await download_bytes_with_progress(client, msg.media, status_msg, "⏱️ Auto DL terdeteksi... 0.00%")
                if not media_bytes:
...
async def _process_dl(event, client, user_id):
    ...
    try:
        media_bytes = await download_bytes_with_progress(client, replied.media, status_msg)
    except Exception as e:
    ...
    try:
        if data == "menu_fitur":
            await query.edit_message_text(
                FITUR_VIP_TEXT,
                parse_mode="Markdown",
                reply_markup=fitur_vip_keyboard(uid)
            )

        elif data == "vip_toggle_auto_dl":
            current = get_auto_dl_view_once(uid)
            set_auto_dl_view_once(uid, not current)
            state = "ON ✅" if (not current) else "OFF ❌"
            await query.answer(f"Auto DL: {state}")
            await query.edit_message_text(
                FITUR_VIP_TEXT + f"\n\n⚙️ Auto DL saat ini: *{state}*",
                parse_mode="Markdown",
                reply_markup=fitur_vip_keyboard(uid)
            )
...
async def _do_backup(query, context):
    try:
        ...
        cur.execute("SELECT user_id, username, full_name, created_at FROM users")
        users_rows = cur.fetchall()

        cur.execute("SELECT user_id, api_id, api_hash, string_session FROM sessions")
        sessions_rows = cur.fetchall()

        cur.execute("SELECT user_id, plan, paid_at, expired_at, is_active FROM subscriptions")
        ...
        lines = []
        lines.append("-- Rams VIP Bot Backup SQL\n")
        lines.append("BEGIN;\n")
        ...
        for r in users_rows:
            lines.append(
                ...
                f"({r[0]}, {esc(r[1])}, {esc(r[2])}, {esc(r[3])});\n"
            )
        for r in sessions_rows:
            lines.append(
                "INSERT INTO sessions (user_id, api_id, api_hash, string_session) VALUES "
                f"({r[0]}, {r[1]}, {esc(r[2])}, {esc(r[3])});\n"
            )
        for r in subs_rows:
            lines.append(
                "INSERT INTO subscriptions (user_id, plan, paid_at, expired_at, is_active) VALUES "
                f"({r[0]}, {esc(r[1])}, {esc(r[2])}, {esc(r[3])}, {r[4]});\n"
            )

        lines.append("COMMIT;\n")
        sql_data = "".join(lines).encode("utf-8")
        bio = io.BytesIO(sql_data)
        bio.name = f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sql"
        ...
        await query.edit_message_text("✅ Backup database berhasil dikirim ke chat admin.", reply_markup=admin_keyboard())
    except Exception as e:
...
def build_app():
    ...
    app.add_handler(CommandHandler("gift", cmd_gift), group=0)
    app.add_handler(CommandHandler("revoke", cmd_revoke), group=0)

    app.add_handler(setup_conv, group=1)
    app.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, admin_message_handler), group=2)
