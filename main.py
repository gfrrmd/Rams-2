...
async def _do_backup(query, context):
    try:
        ...
        dump = io.StringIO()
        for table in ("users", "sessions", "subscriptions", "user_settings"):
            ...
            for r in rows:
                ...
                dump.write(f"INSERT INTO {table} ({','.join(desc)}) VALUES ({','.join(vals)});\n")

        conn.close()
        dump.seek(0)
        data = io.BytesIO(dump.read().encode())
        data.name = "backup.sql"

        await context.bot.send_document(chat_id=ADMIN_ID, document=data, caption="📦 Backup database")
        await query.edit_message_text("✅ Backup berhasil dikirim.", reply_markup=admin_keyboard())
    except Exception as e:
