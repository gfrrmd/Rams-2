...
GUIDE_TEXT = (
    ...
    "Digunakan untuk mendownload media view-once (sekali lihat) atau media dari chat restricted yang tidak bisa di-forward.\n\n"
    "*Cara pakai:*\n"
    ...
    "Digunakan untuk menyalin konten (foto, video, dokumen, teks) dari channel atau grup yang tidak mengizinkan forward.\n\n"
    "*Cara pakai:*\n"
...
async def start_client_for_user(user_id, api_id, api_hash, string_session):
    ...
    async def dl_handler(event):
        ...
        lock = dl_locks.get(user_id) or asyncio.Lock()
...
async def _process_dl(event, client, user_id):
    ...
    await status_msg.delete()
    await client.send_file("me", file=file_obj, caption="✅ Hasil .dl tersimpan.")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "menu_fitur":
            ...
            await query.edit_message_text("🏠 Menu Utama", reply_markup=main_keyboard(uid))
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 Selamat datang di Rams VIP Bot!",
